...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask = None):
        ...
        """
        key_dim = nn.parameter.Parameter(torch.tensor(keys.shape[-1]), requires_grad=False)
...
class MultiheadedAttention(nn.Module):
    def __init__(self,
        ...
        self.output_weights = torch.rand(size=(self.key_dim * self.heads_number, self.embedding_dim))
